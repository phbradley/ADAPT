...
required_columns = ('lineage designable_positions chainseq target_tdinfo '
                    'alignfile'.split())
required_columns.append(args.sort_tag) # need

for col in required_columns:
    assert col in parents.columns, f'Need column {col} in {args.poolfile}'
assert all(exists(x) for x in set(parents.alignfile))
...
if args.run_rfab:
    ...
    rf_targets = wrapper_tools.run_rf_antibody_on_designs(targets, outprefix, model_path=args.rf_antibody_chkpt)
