...
parser.add_argument('--strength_weight', type=float, default= 0.5)
...
parser.add_argument('--sort_tag', required=True,
                    choices = ['pmhc_tcr_pae', 'combo_score', 'combo_score_wtd',
                               'pepspec_combo', 'relax_combo', 'strength_combo'])
...
parser.add_argument('--run_rosetta', action='store_true',
                    help='Whether to run rosetta relax on the designs; if --sort_tag '
...
if args.sort_tag in ['combo_score', 'combo_score_wtd', 'pepspec_combo',
                     'strength_combo']:
    args.run_rfab = True
if args.sort_tag in ['relax_combo']:
    args.run_rosetta = True
if args.sort_tag in ['pepspec_combo']:
    args.pepspec_mpnn = True
...
parents['parent_chainseq'] = parents.chainseq
...
outdir = f'{args.outfile_prefix}_outdir/'
...
if RUN_AF2_SETUP:
    ...
    parents = td2.sequtil.setup_for_alphafold(
        parents, outdir, num_runs=1, use_opt_dgeoms=True, clobber=True,
...
targets.to_csv(f'{outdir}start_targets.tsv', sep='\t', index=False)
...
targets.to_csv(f'{outdir}afold1_results.tsv', sep='\t', index=False)
...
targets = wrapper_tools.run_mpnn(
    ...
    sort_by_peptide_probs=args.pepspec_mpnn,
)
...
for _,row in targets.iterrows():
    ...
    burial = design_stats.get_mhc_vgene_buried_strength(pose, tdinfo)
    dfl.append(burial)

targets['mhc_vgene_buried_strength'] = dfl
...
if args.run_rfab:
    ...
    targets['strength_combo'] = (
        targets.combo_score_wtd + args.strength_weight*targets.mhc_vgene_buried_strength
    )


if args.run_rosetta:
    outprefix = f'{outdir}relax'
    start = timer()
    relax_targets = wrapper_tools.run_rosetta_relax(targets, outprefix)
    relax_time = timer()-start
    targets['relax_time'] = relax_time/targets.shape[0]

    newcols = ('relax_bound_score '
               ...
               'relax_binding_energy_frozen relax_loop_seq '
               'has_loop_disulfide has_pep_disulfide').split()

    for col in newcols:
        targets[col] = list(relax_targets[col])

    targets['relax_combo'] = (targets.pmhc_tcr_pae +
                              100 * targets.relax_peptide_loop_intxn_len_norm +
                              0.1 * targets.relax_binding_energy_frozen)



targets = design_stats.reorient_and_rechain_models(targets)

targets.to_csv(f'{outdir}afold2_results.tsv', sep='\t', index=False)
