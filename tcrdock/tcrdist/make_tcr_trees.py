...
def make_tcr_tree_svg_commands(
    ...
    if single_chain is not None:
        assert single_chain in ['A','B']
    if color_scores is None:
        color_scores = [1.0]*len(tcrs)
    else:
        assert len(color_scores) == len(tcrs)
    ...
    #max_glyphs = 10
    glyph_scale = 1.0 if max_glyphs <= 10 else 10./max_glyphs
    ...
    glyph_region_width = glyph_scale * width_height[0] * 1200.0/2200
    if single_chain:
        glyph_region_width *= 0.5
    ...
    glyph_height = glyph_scale * width_height[1] * 135.0/2000
    ...
    plotter = tcrdist_svg_basic.SVG_tree_plotter(cmap=cmap)
    ...
    node_position, Transform, canvas_tree_min_rmsd, canvas_tree_w_factor = score_trees_devel.Canvas_tree(
        ...
        force_max_rmsd=force_max_rmsd,
        show_colorful_rmsd_bar=show_colorful_rmsd_bar,
    )
    ...
    for ii_ab2, ab2 in enumerate('AB'):
        if single_chain and ab2 != single_chain:
            continue
        if single_chain:
            single_glyph_width = glyph_region_width
        else:
        ...
        for glyph_edge in glyph_edges:
            ...
            if single_chain:
                x0 = upper_left[0] + xmargin + glyph_size_text_width
            else:
