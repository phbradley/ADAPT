    ...
    * tcr_cdrs: list of 8 [start,stop] (inclusive) 0-indexed position lists
    ...
    def from_sequences(
        ...
        if mhc_class is None: # tcr only
            ...
            self.mhc_allele, ident = mhc_util.get_mhc_allele(
                mhc_aseq, organism, return_identity=True)
            self.optional['mhc_identity'] = ident
            self.optional['mhca_identity'] = ident
            self.optional['mhcb_identity'] = ident #hack

        else:
            assert mhc_class == 2
            self.mhc_core = mhc_util.get_mhc_core_positions_class2(
                mhc_aseq, mhc_bseq)
            mhca, identa = mhc_util.get_mhc_allele(
                mhc_aseq, organism, return_identity=True)
            mhcb, identb = mhc_util.get_mhc_allele(
                mhc_bseq, organism, return_identity=True)
            self.mhc_allele = (mhca, mhcb)
            self.optional['mhc_identity'] = 0.5*(identa + identb)
            self.optional['mhca_identity'] = identa
            self.optional['mhcb_identity'] = identb
        ...
        for chain, chainseq in zip('AB', [tcr_aseq, tcr_bseq]):
            ...
            self.optional[f'tcr{chain.lower()}_mismatches'] = res['mismatches']
            offset += len(chainseq)
    ...
    def from_dict(self, D):
        ...
        if 'optional' in D:
            self.optional = copy.deepcopy(D['optional'])
        return self
