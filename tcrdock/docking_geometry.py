...
class DockingGeometry():
    ...
    def from_dict(self, D):
        ...
        val = D.get('tcr_unit_x_is_negative',False)
        if val == 'True':
            val = True
        elif val == 'False':
            val = False
        self.tcr_unit_x_is_negative = bool(val)
        self.mhc_unit_y = D['mhc_unit_y']
        self.mhc_unit_z = D['mhc_unit_z']
        val = D.get('mhc_unit_x_is_negative',False)
        if val == 'True':
            val = True
        elif val == 'False':
            val = False
        self.mhc_unit_x_is_negative = bool(val)
        return self
...
def stub_from_docking_geometry(dgeom):
    ...
    #I believe that axes has the global-space x/y/z unit vectors as row vectors, whereas
    # frame has them as column vectors
...
_loaded_cdr_centroids_in_tcr_frame = {}

def cdr_centroids_from_tcr_stub(
        stub,
        organism = 'human', # not really necessary
):
    global _loaded_cdr_centroids_in_tcr_frame

    if organism not in _loaded_cdr_centroids_in_tcr_frame:
        cenfile = util.path_to_db / f'cdr_centroids_{organism}.txt'
        _loaded_cdr_centroids_in_tcr_frame[organism] = np.loadtxt(cenfile)
        
    cdr_centroids_in_tcr_frame = _loaded_cdr_centroids_in_tcr_frame[organism]
    assert cdr_centroids_in_tcr_frame.shape == (8,3)
    
    centroids = stub['origin'] + cdr_centroids_in_tcr_frame @ stub['axes']
    assert centroids.shape == (8,3)
    return centroids
